...
export class CalendrierComponent implements OnInit {
  ...
  @ViewChild('dateFormat', { static: true }) dateFormat: ElementRef;s
  ...
  ngOnInit(): void {
    ...
    month_picker?.addEventListener('click', () => {
      // Gérer le clic sur le sélecteur de mois
    });
    ...
    month_names.forEach((e, index) => {
      ...
      month.addEventListener('click', () => {
        ...
        generateCalendar(this.currentMonth.value, this.currentYear.value);
    ...
    setInterval(() => {
      ...
      const time = `${timer.getHours().toString().padStart(2, '0')}:${timer.getMinutes().toString().padStart(2, '0')}: ${timer.getSeconds().toString().padStart(2, '0')}`;
